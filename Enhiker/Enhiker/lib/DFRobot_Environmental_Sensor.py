...
import time
#import smbus
from pinpong.board import gboard, I2C #unihiker pinpong
import os
import math
...
class DFRobot_Environmental_Sensor():
  ...
  def __init__(self ,bus ,baud = 9600):
    #if bus != 0:
    try:
      self.i2cbus =  I2C(0)#smbus.SMBus(bus)
    except Exception as e:
      print(f"pinpong is not initialized with begin")
      print(f"Error:{e}")
      raise
...
class DFRobot_Environmental_Sensor_I2C(DFRobot_Environmental_Sensor):
  ...
  def _read_reg(self, reg_addr ,length):
    ...
    try:
      ...
      print(f"SEN0501 read Error:{rslt}")
      rslt = ([0]*length)
    return rslt    
